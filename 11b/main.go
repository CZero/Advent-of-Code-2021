...
func main() {
	var (
		...
		steps      = 2000                // The number of steps to calculate
	...
	for i := 0; i < steps; i++ {
		...
		if ResetFlashed(octopi, maxr, maxc) == 1 {
...
func ResetFlashed(octopi map[coord]int, maxr, maxc int) int {
	var numflashed int
	for r := 0; r <= maxr; r++ {
		for c := 0; c <= maxc; c++ {
	...
	if numflashed == (maxr+1)*(maxc+1) {
		fmt.Println("All flashed!")
		return 1
	}
	return 0
}
