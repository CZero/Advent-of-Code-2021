...
func explore(cavesmap map[string][]string, Solution solution) {
	for _, exit := range cavesmap[Solution.path[len(Solution.path)-1]] {
		...
		onwards.useddouble = Solution.useddouble
		// Copy done
		...
		case exit == "start": // Niet gebruiken
		case !unicode.IsUpper(rune(exit[0])) && onwards.visited[exit] > 0 && onwards.useddouble == true: // Kleine grot reeds bezocht
		...
		case !unicode.IsUpper(rune(exit[0])) && onwards.visited[exit] > 0 && onwards.useddouble == false: // Kleine grot 1 keer bezocht, maar nog geen dubbele ingezet.
			onwards.path = append(onwards.path, exit)
			onwards.visited[exit]++
			onwards.useddouble = true
			explore(cavesmap, onwards)
